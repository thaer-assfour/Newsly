...
class _WelcomeScreenState extends State<WelcomeScreen> {
  List pageId = [
    ...
    3,
    4
  ]; // number of pages and connected to name of background in assets folder.
  List pageTitle = [
    "Welcome to \n Newsly \n App",
                  ...
                  Container(
                    ...
                    child: RaisedButton(
                      ...
                      onPressed: () async {
                        ...
                        prefs.setBool('runBefore',
                            true); // set// value to not shown welcome screen if the program loaded before
...
class PaintDrawer extends CustomPainter {
  @override
  void paint(Canvas canvas, Size size) {
    Paint paint = Paint();
    paint.color = themeColor[1].withOpacity(0.9);
    Path path = Path();
    path.lineTo(0, 0);
    path.lineTo(size.width, 0);
    path.lineTo(size.width, size.height * 0.005);
    path.quadraticBezierTo(
        size.width / 2, size.height * 0.05, 0, size.height * 0.005);
    path.lineTo(0, size.height * 0.005);
    path.close();
